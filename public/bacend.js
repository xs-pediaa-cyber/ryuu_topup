...
router.post("/deposit/status", requireLogin, async (req, res) => {
  ...
  const { id } = req.body; // Ini payinaja_trx_id
  ...
  try {
    ...
    // Cari data di history deposit user untuk ambil nominal saldo bersih (get_balance)
    const history = user.history_deposit.find(h => h.id === id);

    if (history && history.status === "pending") {
        if (status === "success") {
            // Update status history jadi success
            await editHistoryDeposit(user._id, id, "success");
            // Tambah saldo user berdasarkan get_balance yang disimpan saat create
    ...
    return res.status(200).json({
      ...
      data: {
        ...
        fee: history ? history.fee : d.fee,
        get_balance: history ? history.get_balance : d.amount_requested,
